# Copy from https://github.com/Manudecara/Algo-Trading

want <- c("quantmod","tseries", "dplyr", "knitr", "lubridate", "class", "e1071", "C50", "PerformanceAnalytics", "gmodels", "data.table")
...
GOOG <- na.omit(merge(GOOG, CCI(GOOG[, 2:4])))
GOOG <- na.omit(merge(GOOG, ADX(HLC(GOOG))))
GOOG$return <- dailyReturn(Cl(GOOG))
GOOG <- GOOG[,-6] # delete adjusted price
GOOG <- GOOG[,-c(12:14)]
